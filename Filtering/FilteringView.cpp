...
void CFilteringView::OnProblem3()
{
	...
	for (r=0; r<m_height; r++) {
		for (c=0; c<m_width; c++) {
			Gx[r*m_width+c] = Gy[r*m_width+c] = 0;
			for(k=0; k<3; k++){					
				for(l=0; l<3; l++){
					Gx[r*m_width+c] += Sx[k][l]*m_imageBits[(r+k-w0/2)*m_width+(c+l-w0/2)];
					Gy[r*m_width+c] += Sy[k][l]*m_imageBits[(r+k-w0/2)*m_width+(c+l-w0/2)];
				}
			}
...
void CFilteringView::AverageFiltering(int w0)
{
	...
	for (r=0; r<m_height; r++) {
		for (c=0; c<m_width; c++) {
			...
			for(k=0; k<w0; k++){
				//if((r<w0/2 && k<w0/2-r) || (r>w0/2 && k>w0/2+r)) continue;
				for(l=0; l<w0; l++){
					//if((c<w0/2 && l<w0/2-c) || (c>w0/2 && l>w0/2+c)) continue;
					mean += kernel[k][l]*imageBits[(r+k-w0/2)*m_width+(c+l-w0/2)];
				}
			}
		...
		for (c=0; c<m_width; c++) {
			...
			if(c==0){
				...
				for(k=0; k<w0; k++){
					...
					for(l=0; l<w0; l++){
						//if((c<w0/2 && l<w0/2-c) || (c>w0/2 && l>w0/2+c)) continue;
						mean += kernel[k][l]*m_imageBits[(r+k-w0/2)*m_width+(c+l-w0/2)];
					}
				}
			}
			else{
				for(k=0; k<w0; k++){
					mean -= kernel[k][0]*m_imageBits[(r+k-w0/2)*m_width+(c-1-w0/2)];
					mean += kernel[k][w0-1]*m_imageBits[(r+k-w0/2)*m_width+(c+w0/2)];
				}
			}
...
void CFilteringView::MedianFiltering(int w0)
{
	...
	for (r=0; r<m_height; r++) {
		for (c=0; c<m_width; c++) {
			...
			// Problem 2-a1,a2,a3: imageBits 에 Median Filtering 처리를 하여 imageBitsFiltered 에 할당한다
			// QuickMedian() 함수를 이용하여 Sorting 한다.
			for(k=0; k<w0; k++){
				for(l=0; l<w0; l++){
					medarray[k*w0+l] = m_imageBits[(r+k-w0/2)*m_width+(c+l-w0/2)];
				}
			}
